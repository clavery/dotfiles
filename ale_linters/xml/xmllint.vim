...
function! ale_linters#xml#xmllint#GetCommand(buffer) abort
  let l:schemas = {'http://www.demandware.com/xml/impex/order/2006-10-31' : 'order.xsd' }

  let l:matches = matchlist(getline(2), 'xmlns="\(.\+\)"')
  echo 'matches ' . len(l:matches)
  if len(l:matches) > 0
    return ale#Escape(ale_linters#xml#xmllint#GetExecutable(a:buffer))
          \   . '--schema /Users/clavery/code/dwre/dwre-dwre-tools/dwre_tools/schemas/' . l:schemas[l:matches[1]] . ' ' . ale#Var(a:buffer, 'xml_xmllint_options')
          \   . ' --noout -'
  else
    return ale#Escape(ale_linters#xml#xmllint#GetExecutable(a:buffer))
